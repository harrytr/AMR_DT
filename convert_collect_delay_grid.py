...
def main() -> int:
    ...
    delay_dirs = sorted([p for p in BASE.iterdir() if p.is_dir() and p.name.startswith("delay_")])
    total_all = 0
    ...
    for dd in delay_dirs:
        out_dir = OUT_BASE / dd.name
        out_dir.mkdir(parents=True, exist_ok=True)

        sims = sorted([p for p in dd.iterdir() if p.is_dir() and p.name.startswith("sim_")])
        total = 0
        for sd in sims:
            run_convert(sd)
        ...
        total_all += total
        print(f"DELAY_DONE {dd.name}: sims={len(sims)} pt_files={total} out={out_dir}", flush=True)
